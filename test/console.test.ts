...
describe("Console Transport", () => {
  ...
  it("should log to console using correct method", () => {
    ...
    // DEBUG
    transport.write({ ...mockEntry, level: 20, levelName: "DEBUG" });
    ...
    // FATAL (uses error)
    transport.write({ ...mockEntry, level: 60, levelName: "FATAL" });
    expect(errorSpy).toHaveBeenCalledTimes(2);

    // TRACE (fallthrough to log)
    transport.write({ ...mockEntry, level: 10, levelName: "TRACE" });
